...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $check_query = "SELECT id FROM manager WHERE regno = ?";
    ...
    if (mysqli_num_rows($check_result) > 0) {
        ...
        // Insert manager
        $query = "INSERT INTO manager (username, regno, name, father, mother, asession, dob, age, doj, gender, email, mob, state, dis, pincode, category, marital_status, qualification, aadhar, othermob_no, pass, address, bank, bank_branch, ifsccode, accountno, bid, status, date, reg_type) 
                  VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1)";
        $stmt = mysqli_prepare($con, $query);
        mysqli_stmt_bind_param(
            $stmt,
            "issssisissssssssssssssssssiis",
            $username,
            $regno,
            $name,
            $father,
        ...
        if (mysqli_stmt_execute($stmt)) {
            ...
            logActivity('create_manager', 'manager', $manager_id, null, json_encode($_POST));
            header('Location: list.php?success=added');
            exit;
        } else {
            $error = 'Failed to add manager: ' . mysqli_error($con);
        }
    }
}
...
$result = mysqli_query($con, "SELECT MAX(CAST(SUBSTRING(regno, 3) AS UNSIGNED)) as max_num FROM manager WHERE regno LIKE 'MG%'");
$row = mysqli_fetch_assoc($result);
$next_num = ($row['max_num'] ?? 0) + 1;
$auto_regno = 'MG' . str_pad($next_num, 4, '0', STR_PAD_LEFT);
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-12 mt-4">
                            <button type="submit" class="btn btn-primary">
                                <i class="fas fa-save me-2"></i>Save Manager
                            </button>
