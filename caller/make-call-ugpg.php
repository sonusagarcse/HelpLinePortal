...
$prev_query = "SELECT nextdate, pdate, des FROM mquery WHERE studentid = ? AND query_type = 'UG_PG' ORDER BY id DESC LIMIT 1";
...
$data['remarks'] = $student['ugpg_caller_remark'];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $insert_query = "INSERT INTO mquery (callerid, studentid, bid, des, pdate, nextdate, remarks, date, status, query_type) 
                     VALUES (?, ?, ?, ?, ?, ?, ?, NOW(), ?, ?)";
    ...
    if (mysqli_stmt_execute($insert_stmt)) {
        ...
        $reg_update_sql = "ugpg_caller_remark = ?, ugpg_assigned_caller = ?";
        $params = [$remarks, $caller_id];
        $types = "si";

        if ($is_rejected) {
            $reg_update_sql .= ", ugpg_status = 2";
        } elseif (isset($_POST['status'])) {
            $reg_update_sql .= ", ugpg_status = 1";
        } else {
            $reg_update_sql .= ", ugpg_status = 0";
        }
...
?>
                ...
                <div class="premium-card">
                    ...
                    <div class="timeline">
                        <?php
$history_query = "SELECT * FROM mquery WHERE studentid = ? AND query_type = 'UG_PG' ORDER BY id DESC";
                    ...
                    <form method="POST" action="">
                        ...
                        <div class="bg-light p-4 rounded-4 mb-4 border border-white">
                            ...
                            <div class="form-check form-switch mb-3">
                                <input class="form-check-input" type="checkbox" role="switch" name="status" id="completed" <?php echo (isset($student['ugpg_status']) && $student['ugpg_status'] == 1) ? 'checked' : ''; ?>>
                                <label class="form-check-label fw-semibold" for="completed">Admission Done</label>
                            </div>

                            <hr class="my-3 opacity-10">

                            <div class="form-check">
                                <input type="checkbox" name="reject_student" class="form-check-input" id="reject" onchange="toggleReject(this)" <?php echo (isset($student['ugpg_status']) && $student['ugpg_status'] == 2) ? 'checked' : ''; ?>>
                                <label class="form-check-label text-danger fw-bold" for="reject">
                                    <i class="fas fa-user-times me-1"></i> Reject Only
                                </label>
                            </div>
                        </div>
